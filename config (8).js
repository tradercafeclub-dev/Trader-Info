...
// ── API Endpoint + Keys ───────────────────────────────────────
var WORKER_URL = 'https://tradercafeclub.tradercafeclub.workers.dev';
var SYSTEM_WALLET_DEFAULT = { label:'กระเป๋าหลัก', addr:'0xfd1A5bc82603a702a9AF38426FfAC0d6BE5aEeef', qr:'https://i.ibb.co/d0T7YJfy/line-oa-chat-260203-125428.jpg', active:true };
var LS_ADMINS  = 'tcc_admins';
var LS_WALLETS = 'tcc_wallets';
var LS_SESSION = 'tcc_session';
...
// ── Cache + API Layer ─────────────────────────────────────────
var _cache={};
var _FRESH_TTL  = 90_000;   // 90 วินาที — คืนทันที
var _STALE_TTL  = 300_000;  // 5 นาที   — ใช้ stale + revalidate bg
var _BG_PENDING = new Set(); // ป้องกัน bg fetch ซ้ำซ้อน

var _cachePut = function(key,list){ _cache[key]={ data:list, ts:Date.now() }; };
var _cacheAge = function(key){ var c=_cache[key]; return c ? Date.now()-c.ts : Infinity; };
var _cacheBust = function(tid){ for(var k of Object.keys(_cache)) if(k.startsWith(tid)) delete _cache[k]; };

var _nocoFetch = async function(url, retries=3, delay=1200){
  while(retries-->0){
    try{
...
var nocoGet = async function(tid,params=''){
...
var nocoPost = async function(tid,body){
...
var defaultPermissions = function(){
  const p = {};
...
var can = function(module, action){
...
var SS_MEMBERS='tcc_ss_members', SS_TEAMS='tcc_ss_teams', SS_PAYMENTS='tcc_ss_payments';
var SS_TTL = 10 * 60 * 1000; // 10 นาที
var ssSet = function(k,d){try{sessionStorage.setItem(k,JSON.stringify({d,t:Date.now()}));}catch(e){}};
var ssGet = function(k){try{var r=sessionStorage.getItem(k);if(!r)return null;var o=JSON.parse(r);if(Date.now()-o.t>SS_TTL){sessionStorage.removeItem(k);return null;}return o.d;}catch(e){return null;}};
var ssClear = function(k){try{if(k)sessionStorage.removeItem(k);else[SS_MEMBERS,SS_TEAMS,SS_PAYMENTS].forEach(x=>sessionStorage.removeItem(x));}catch(e){}};
var applyDashboardStats = function(){
...
var MEMBERS=[], TEAMS=[], PAYMENTS=[];
var currentMemberPayments=[], selectedAdminMember=null, tradeRows=[];
